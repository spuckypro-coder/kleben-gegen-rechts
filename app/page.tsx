...
export default async function Home() {
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <section className="relative overflow-hidden halftone flex items-center">
        ...
        <div className="relative z-20 max-w-6xl mx-auto px-4 py-10 text-center w-full">
          ...
          <h1 className="text-4xl sm:text-6xl md:text-8xl font-black uppercase leading-none mb-6">
            ...
            <span className="block text-cyan-500">{c.hero_title_3}</span>
          </h1>
          <p className="text-gray-400 text-lg max-w-xl mx-auto mb-10 font-medium">
            {c.hero_subtitle}
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center px-4">
            <Link href="/galerie" className="px-8 py-4 bg-red-600 text-white font-black uppercase tracking-widest hover:bg-red-500 transition-colors" style={{ boxShadow: "4px 4px 0px #06b6d4" }}>
              {c.hero_btn_galerie}
            </Link>
      ...
      {latestPosts.length > 0 && (
        <section className="max-w-6xl mx-auto px-4 py-10 md:py-20">
          <div className="flex items-center justify-between mb-10">
            ...
            <Link href="/blog" className="text-cyan-500 font-bold uppercase text-sm tracking-widest hover:underline">
              Alle ansehen →
            </Link>
          </div>
      ...
      {latestImages.length > 0 ? (
        <section className="max-w-6xl mx-auto px-4 py-10 md:py-20">
          <div className="flex items-center justify-between mb-10">
            ...
            <Link href="/galerie" className="text-cyan-500 font-bold uppercase text-sm tracking-widest hover:underline">
              Alle ansehen →
            </Link>
          </div>
        ...
        <div className="flex whitespace-nowrap" style={{ animation: "marquee 25s linear infinite" }}>
          {Array.from({ length: 8 }).map((_, i) => (
            <span key={i} className="text-white font-black uppercase text-base sm:text-xl mx-6 sm:mx-10 shrink-0">
              {c.marquee_text} <span className="text-cyan-400 mx-4">✊</span>
            </span>
          ))}
        </div>
      </section>
      ...
      {featuredProduct && (
        <section className="max-w-6xl mx-auto px-4 py-10 md:py-20">
          <div className="grid md:grid-cols-2 gap-0 border-2 border-cyan-500" style={{ boxShadow: "8px 8px 0px #ff0033" }}>
            {/* Bild */}
            <div className="relative aspect-video md:aspect-square bg-gray-900">
              ...
              />
              <div className="absolute top-0 left-0 px-4 py-2 bg-cyan-500 text-black font-black uppercase text-xs tracking-widest">
                ★ Highlight
              </div>
            </div>
            {/* Info */}
            <div className="bg-gray-950 p-6 md:p-10 flex flex-col justify-between">
              <div>
                ...
                <div className="text-3xl sm:text-5xl font-black text-cyan-500 mb-6 md:mb-8">
      ...
      <section className="max-w-6xl mx-auto px-4 py-10 md:py-20">
        <div className="grid md:grid-cols-2 gap-8 items-center">
          <div>
            <div className="inline-block mb-4 px-3 py-1 bg-cyan-500 text-black text-xs font-black uppercase">{c.shop_badge}</div>
            <h2 className="text-4xl font-black uppercase mb-4">
              <span className="text-red-500">{c.shop_title}</span>
            </h2>
            <p className="text-gray-400 mb-6">{c.shop_text}</p>
            <Link href="/shop" className="inline-block px-6 py-3 bg-cyan-500 text-black font-black uppercase tracking-widest hover:bg-cyan-400 transition-colors">
              Zum Shop →
            </Link>
          </div>
